...
- `scripts/`
  ...
  - `scripts/powershell/<role>/` 아래에 역할별 분류 기준을 둡니다.
  - 루트 `scripts/*.ps1`는 점진적으로 줄이고, 이동이 끝난 항목은 `scripts/powershell/<role>/`를 기준으로 사용합니다.
- `skills/`
  - repo-local 스킬 엔트리
- `templates/`
...
이 스캐폴드를 기존 프로젝트에 클론하거나 복사한 뒤, 넓은 코드 변경 전에 현재 구조를 먼저 시각화할 수 있습니다.
...
새 작업 시작과 오케스트레이션 파이프라인 성공 후에는 이 맵이 자동으로 갱신됩니다. 자동 갱신을 건너뛰려면 `.\scripts\run-orchestration.ps1 -SkipCodeFlowMap`을 사용합니다.

브라우저 보드는 `docs/code-flow-board.html`을 열어서 봅니다.

실시간 작업 보드는 React + ELKJS 기반 dev server로 실행합니다.
...
이 모드는 파일 변경을 감시해 `code-flow` 데이터를 다시 생성하고, 열린 보드 화면을 자동 갱신합니다.

다른 프로젝트를 바라보며 보조 보드로 쓸 때는 대상 루트를 지정합니다. `start-scaffold` 폴더는 분석 대상에서 제외됩니다.

```powershell
.\scripts\start-code-flow-board.ps1 -TargetRoot "C:\path\to\project" -Port 5179
```

노드 설명은 로컬 AI CLI로 생성합니다. 설명 생성 명령에는 `-AiCommand`가 필수입니다.

```powershell
.\scripts\enrich-code-flow-descriptions.ps1 -Root "C:\path\to\project" -AiCommand "claude -p --max-budget-usd 0.20"
...
- 기본 기준 환경은 Windows PowerShell 또는 PowerShell 7(`pwsh`)입니다.
- WSL에서도 사용할 수 있지만, `pwsh`가 설치되어 있어야 합니다.
- Git hook은 이제 `pwsh`가 있으면 WSL/Linux에서 먼저 사용하고, 없으면 Windows `powershell.exe`로 fallback합니다.
- 전역 skill 경로는 OS별로 다를 수 있으므로, 문서에서는 Windows 절대경로 대신 skill 이름 기준으로 참조합니다.
- 실행 전략은 두 가지 패턴으로 관리합니다:
  - `powershell-bridged`: 현재 기본 패턴. Windows와 WSL에서 PowerShell 진입점을 공용으로 사용합니다.
  - `native-wsl-linux`: Linux/WSL 친화 패턴. bash 기반 진입점을 별도로 두는 확장 방향입니다.
- 자세한 기준은 `docs/modes/shared/runtime-environment-patterns.md`를 따릅니다.
- 스크립트 구조는 점진적으로 `scripts/shared/`, `scripts/powershell/`, `scripts/bash/`로 나누는 방향을 따릅니다.
...
```powershell
.\scripts\powershell\bootstrap\start-task.ps1 -TaskName "my first mvp" -Agent codex -Pack start
```

공용 minimum-goal 스킬 시작:

```powershell
.\scripts\powershell\skills\skill-minimum-goal.ps1 -Agent codex -Stage start -TaskName "my task" -Pack start
```

필요한 문서만 골라서 context 로드:

```powershell
.\scripts\powershell\context\select-context-pack.ps1 -Agent codex -Pack implement
```
...
```bash
pwsh -NoProfile -File ./scripts/powershell/bootstrap/init-project.ps1
pwsh -NoProfile -File ./scripts/run-code-rules-checks.ps1
pwsh -NoProfile -File ./scripts/powershell/skills/skill-minimum-goal.ps1 -Agent codex -Stage start -TaskName "my task" -Pack start
```

Linux/WSL native 예시:

```bash
./scripts/bash/select-context-pack.sh --agent codex --pack start
./scripts/bash/run-code-rules-checks.sh
./scripts/bash/skill-minimum-goal.sh --agent codex --stage start --task-name "my task" --pack start
```
...
- `scripts/analyze-code-flow.ps1`
  - 현재 프로젝트의 파일 역할과 정적 참조를 분석해 Mermaid/JSON 플로우 맵을 생성합니다.
